...
class RequestPlugin
  ...
  def request(m, username, email, server, port, reqserver = nil)
    ...
    unless reqserver.nil?
      unless $config["zncservers"].keys.include? reqserver.downcase
        m.reply "Error: #{reqserver} is not a valid leprechuan.net server. " + \
  ...
  def topic(m, topic)
    return unless m.channel == "#leprechuan-ops"
    command = "TOPIC"
    ...
    $bots.each_value do |bot|
      bot.irc.send("PRIVMSG ChanServ :#{command} #leprechuan #{topic}")
    end
    m.reply "done!"
  end
  ...
  def broadcast(m, text)
    return unless m.channel == "#leprechuan-ops"
    $bots.each_value do |bot|
      bot.irc.send("PRIVMSG #leprechuan :#{Format(:bold, "[BROADCAST]")} #{text}")
    end
  ...
  def help(m)
    ...
    m.reply "#{Format(:bold, "Syntax: !request <user> <email> <server> [+]<port> [leprechuan.net server]")}. Parameters in brackets are not required. A + before the port denotes SSL. This command can be issued in a private message."
    m.reply "For example, a user called foo with an email of foo@bar.tdl who wants a bouncer for Interlinked on our newyork server would issue: " + \
             Format(:bold, "!request foo foo@bar.tdl irc.freenode.net 6667 newyork")
  end

  def web(m)
    m.reply "http://leprechuan.net"
  end

  def setup(m)
    m.reply "http://leprechuan.net/setup.html"
  end

  def adminmsg(text)
    $adminbot.irc.send("PRIVMSG #leprechuan-ops :#{text}")
  end
